...
const queryGames = async (query) => {
    ...
    allowedQueries.forEach(currentQuery => {
        if (query.hasOwnProperty(currentQuery)) {
            const arrayQuery = query[currentQuery].split(" ");
            const adjustedQuery = arrayQuery.map((query) => {
                if (query !== "of" && query !== "the") {
                    return query[0].toUpperCase() + query.substring(1)
                } else {
                    return query
                }
            }).join(" ");
            firestoreQuery = firestoreQuery.where(currentQuery, '>=', adjustedQuery).where(currentQuery, '<=', adjustedQuery + '\uf8ff')
        }
    })
